...
public class BlockInfiltratorController extends BlockMachinePart implements IActivatable {
	
	public BlockInfiltratorController() {
		super();
	...
	public TileEntity createNewTileEntity(World world, int metadata) {
		return new TileInfiltratorController();
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!world.isRemote) {
			...
			if (tile instanceof TileInfiltratorController controller) {
				if (controller.isMultiblockAssembled()) {
					NCCriterions.INFILTRATOR_ASSEMBLED.trigger((EntityPlayerMP) player);
