...
public class BlockElectrolyzerController extends BlockMachinePart implements IActivatable {
	
	public BlockElectrolyzerController() {
		super();
	...
	public TileEntity createNewTileEntity(World world, int metadata) {
		return new TileElectrolyzerController();
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!world.isRemote) {
			...
			if (tile instanceof TileElectrolyzerController controller) {
				if (controller.isMultiblockAssembled()) {
					NCCriterions.ELECTROLYZER_ASSEMBLED.trigger((EntityPlayerMP) player);
