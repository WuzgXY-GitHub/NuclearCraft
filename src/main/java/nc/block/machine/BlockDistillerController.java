...
public class BlockDistillerController extends BlockMachinePart implements IActivatable {
	
	public BlockDistillerController() {
		super();
	...
	public TileEntity createNewTileEntity(World world, int metadata) {
		return new TileDistillerController();
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!world.isRemote) {
			...
			if (tile instanceof TileDistillerController controller) {
				if (controller.isMultiblockAssembled()) {
					NCCriterions.DISTILLER_ASSEMBLED.trigger((EntityPlayerMP) player);
